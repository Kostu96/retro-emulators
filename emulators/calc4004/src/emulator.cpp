...
Emulator::Emulator() :
    ...
    cpu_.mapWriteSRCRegisterCallback([this](u8 cm_ram, u8 value) {
        utils::assert(cm_ram == 1, "Invalid cm_ram = {}", cm_ram);
    ...
    const char* rom_filename = "data/rom.asm";
    std::string rom_source = utils::readFile(rom_filename);
    Assembler40xx assembler{ rom_source };
    [[maybe_unused]] auto status = assembler.assemble();
    utils::assert(status == Assembler40xx::Status::Success, "Could not assemble {}", rom_filename);
    ...
    while (true) {
        ...
        utils::assert(address + data_length <= ROM_SIZE, "");
        ...
        utils::assert(checksum_accumulator == 0, "Invalid checksum");
    }
}

u8 Emulator::read_ROM(u16 address) const {
    utils::assert(address < ROM_SIZE, "Invalid address = {}", address);
    return ROM_[address];
}

void Emulator::write_ROM_IO(u8 value) {
    utils::assert((value & 0xF0) == 0, "Invalid value = {}", value);
    utils::assert(ROM_SRC_reg_ < 2, "Invalid ROM_SRC_reg_ = {}", ROM_SRC_reg_);
    ...
    case 1: {// shift register
        ...
        if (!last_clock_state && clock_state) {
            shift_register_.clock((value >> 2) & 1);
        }
        last_clock_state = clock_state;
        shift_register_.enable(value & 1);
    } break;
    }
}
...
u8 Emulator::read_RAM_data(u8 cm_ram) const {
    ...
    utils::assert(address < RAM_DATA_SIZE, "Invalid address = {}", address);
...
u8 Emulator::read_RAM_status(u8 cm_ram, u8 char_idx) {
    ...
    utils::assert(address < RAM_STATUS_SIZE, "Invalid address = {}", address);
...
void Emulator::write_RAM_output(u8 cm_ram, u8 value) {
    utils::assert(cm_ram == 1, "Invalid cm_ram = {}", cm_ram);
    utils::assert((value & 0xF0) == 0, "Invalid value = {}", value);
    utils::assert((RAM_SRC_reg_ & 0xFC) == 0, "Invalid RAM_SRC_reg_ = {}", RAM_SRC_reg_);

    switch (RAM_SRC_reg_) {
    case 0: utils::assert(false, "Unimplemented"); break;
    case 1: utils::assert(false, "Unimplemented"); break;
    case 2:
        utils::assert(false, "Unimplemented");
        break;
    case 3: utils::assert(false, "Unimplemented"); break;
    }
}
