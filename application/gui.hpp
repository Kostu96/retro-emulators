...
struct MemoryView
{
    ...
    {
        Sizes s;
        calcSizes(s, core->getMemorySizes()[memoryIndex], base_display_addr);
        ImGui::SetNextWindowSize(ImVec2(s.WindowWidth, s.WindowWidth * 0.60f), ImGuiCond_FirstUseEver);
        ImGui::SetNextWindowSizeConstraints(ImVec2(0.0f, 0.0f), ImVec2(s.WindowWidth, FLT_MAX));

        if (ImGui::Begin(title, &open, ImGuiWindowFlags_NoScrollbar))
        {
            drawContents(core, memoryIndex, base_display_addr);
    ...
    void drawContents(const std::shared_ptr<EmulatorCore>& core, size_t memoryIndex, size_t base_display_addr = 0x0000)
    {
        if (cols < 1)
            cols = 1;
